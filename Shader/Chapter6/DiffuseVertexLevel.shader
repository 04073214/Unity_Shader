Shader "Tutorial/Chapter 6/Diffuse Vertex-Level" 
{
    Properties
    {
    ...
    SubShader
    {
        Pass
        {
            ...
            v2f vert(a2v v)
            {
                ...
                //顶点法线从模型空间转换到世界空间
                fixed3 worldNromal=normalize(mul(v.normal,(float3x3)unity_WorldToObject));

                //世界坐标下该Pass处理的光源方向
                fixed3 worldLight=normalize(_WorldSpaceLightPos0.xyz); 

                //_LightColor0为该Pass处理的光源的颜色，diffuse为漫反射光照
                fixed3 diffuse=_LightColor0.rgb*_Diffuse.rgb*saturate(dot(worldNromal,worldLight));
